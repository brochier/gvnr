...
class Model(object):
    def __init__(self,
        ...
        self.vectorI = tf.placeholder(tf.float32, shape=[None, None], name="inputI")
        self.IW = tf.Variable(
        ...
        self.vectorJ = tf.placeholder(tf.float32, shape=[None, None], name="inputJ")
        self.JW = tf.Variable(
        ...
        wFactor = tf.minimum(1.0, tf.pow(tf.div(tf.exp(self.Xij), 10), 0.75))
        self.loss = tf.reduce_sum(tf.multiply(wFactor, dist), name="loss")

        tf.summary.scalar("loss", self.loss)
        self.global_step = tf.Variable(0, trainable=False, name="global_step")
        self.learnRate = tf.Variable(learn_rate, trainable=False, name="learnRate")
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learn_rate, name="optimizer").minimize(
...
class GloVe:
    ...
    def generate_batches(self):
        ...
        for ind in self.chuncker(list(range(0, len(data))), self.batch_size):
            yield rows[ind], cols[ind], data[ind]

    def fit(self, X, embedding_size = 80, batch_size = 128, n_epochs = 2, learn_rate=0.001):
        X.eliminate_zeros()
        ...
        self.embedding_size = embedding_size
        self.num_batches = math.ceil(self.links_count / self.batch_size) * n_epochs
        self.learn_rate = learn_rate
