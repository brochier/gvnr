...
class gvnrt:
    ...
    def fit(self,
        ...
        logger.debug(" ".join([str(v) for v in ["Shape of features matrix : ", M.shape]]))
    ...
    def predict(self, x, embedding='IJ'):
        if embedding == 'IJ':
            return np.hstack([x.dot(self.I_matrix), x.dot(self.I_matrix)])
        elif embedding == 'I':
            return x.dot(self.I_matrix)
        elif embedding == 'J':
            return x.dot(self.J_matrix)
